...
calc_TAK_attack_rates = function(VCD, titre = F) {
...
calc_TAK_hosp_rates = function(hosp, VCD) {
...
extract_TAK_model_results = function(fit_ext){
...
plot_TAK_attack_rate = function(VCD,
                            hosp,
                            file_path, 
                            hospital = F,
                            AR) {
  ...
  if (hospital == F) {
    ...
    fil = "2"
    AR_data = calc_TAK_hosp_rates(VCD = VCD, hosp = hosp)
...
plot_TAK_VE  = function(file_path,
                    hospital = F,
                    serostatus,
                    VE = NULL
) {
...
plot_TAK_titres = function(file_path, n = NULL){
...
plot_TAK_output = function(VCD=NULL,
                       hosp=NULL,
                       titre_model=T,
                       file_path,
                       AR = NULL,
...
# plot attack rates 
  ...
  # plot VE
  plot_TAK_VE(file_path = file_path,
...
# if titre plot plot imputed titres 
  if (titre_model == T) {
    plot_TAK_titres(file_path = file_path,n=n) }}
