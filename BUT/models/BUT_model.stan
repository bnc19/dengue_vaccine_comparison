...
transformed parameters{
  ...
  // contrain p by lambda 
  lambda_m = mean(lambda_K); // mean across D1 and D2 
...
for(b in 1:B)
 for(v in 1:V)
   for(k in 1:K)
    for(j in 1:J){
  pD_BVK[b,v,k] = sum(Sy_BVKJ[b,v,k, ]) / Sy ; // serotype serostatus trial
  pD_BVJ[b,v,j] = sum(Sy_BVKJ[b,v, ,j]) / Sy ; // age serostatus trial 
  }
...
// priors
  ...
  w ~ normal(1,2);
  lc[1,] ~ normal(5.41,1);
  for(c in 2:C)  lc[c,] ~ normal(6.67,1);
  omega ~ normal(0,2);
  kappa ~  normal(0,2);
  beta ~ normal(0,2);
