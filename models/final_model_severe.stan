...
array[TR, B, K] real<lower=0> mu; // trial specific titres at T0
   ...
   // Dengvaxia data 
   array[D_De] int<lower=0> SE_SN_D_De;  // total severe SN over time
   int<lower=0> SE_De;                   // total severe (not published over time)
   ...
   array[(B*V*J_De*K)-8] int<lower=0> SE_BVKJ_De ;  // severe
   array[V*J_De,D_De]     int<lower=0> SE_SN_VJD_De;   // severe
   array[J_De]         int<lower=0> SE_SPvJ_De ;    // severe
...
// Dengvaxia flag 
int<lower = 0, upper = 1> psi_J;            // 0 for single psi, 1 for age psi
...
transformed data {
 ...
 array [V,D_De] int pop_HOSP_SN_VD_De;
  ...
  for (v in 1:V)
    for (j in 1:J_De) {
      ...
      for (d in 1:D_De){
        ...
        pop_HOSP_SN_VD_De[v,d] = sum(pop_HOSP_BVJD_De[1,v, ,d]) ; // SN by vaccine arm over time 
    }
}
...
array [J_De] real<lower = 0, upper = 1> psi ;  // prob severe for Dengvaxia 
...
// ######################  DENGVAXIA PARAMETERS TO ACCESS ######################
  ...
  array[B,V,K,J_De,D_De] real<lower = 0> Se_BVKJD_De;         // all cases 
  array[B,V,J_De,D_De]  real<lower = 0> Se_BVJD_De;
  ...
  vector [B*V*K*J_De-8]  mSe_BVKJ_De ;  
  matrix [V*J_De,D_De]   mSe_SN_VJD_De ;    
  vector [J_De]       mSe_SPvJ_De ;  
  ...
  array[D_De] real<lower=0, upper=1> pSe_SN_D_De; 
  real<lower=0, upper=1> pSe_De; 
...
array [B, V, K, J_De, T_De] real Se1;
array [B, V, K, J_De, T_De] real Se2;
array [B, V, K, J_De, T_De] real Se34;
  
// Aggregate disease metrics
array[B, V, K, J_De] real Di;
array[B, V, K, J_De, D_De] real H;
array[B, V, K, J_De, D_De] real Se;
...
array [B,V,D_De] real Se_BVD;
array [B,V]   real Se_BV;
array [B,D_De]   real Se_BD;
array [D_De]     real Se_SN_D; // SN over time 
array [D_De]     real Se_D;
real Se_total; 
...
array [B,V,J_De, D_De] real pSe_BVJD;
array [B,V,K,J_De] real pSe_BVKJ;
array [B,V,J_De]   real pSe_BVJ; 
...
if(psi_J ==1){
  for(j in 1:J_De) psiT[j] = psi[j];
} else {
 for(j in 1:J_De) psiT[j] = psi[1];
}
    ...
    for(j in 1:J_De) {
      for(t in 1:T_De){
         ...
         for(k in 1:K){
           ...
           Se1[b,v,k,j,t] = Inc1[b,v,k,j,t] * rhoT[k] * gammaT[k] * RR_hosp_De[1,v,k,j,t] * psiT[j] ; 
           if(include_eps == 0){
             Se2[b,v,k,j,t] = Inc2[b,v,k,j,t] * rhoT[k] * RR_hosp_De[2,v,k,j,t] * psiT[j]  ; 
             } else {
               Se2[b,v,k,j,t] = Inc2[b,v,k,j,t] * rhoT[k] * RR_hosp_De[2,v,k,j,t] * psiT[j]  * epsilon;  
               }
               Se34[b,v,k,j,t] = (Inc3[b,v,k,j,t] + Inc4[b,v,k,j,t]) * rhoT[k] * gammaT[k] * phiT[k] * RR_hosp_De[3,v,k,j,t] * psiT[j]  ; 
...
for(b in 1:B)
 for(v in 1:V)
  for(k in 1:K)
   for(j in 1:J_De)  // hosp is severe cases plus prop non-severe (delta) hosp 
     for(t in 1:T_De){  
       
            if(delta_KJ[2] != 2){

                H1[b,v,k,j,t] =  (deltaT[k] * D1[b,v,k,j,t])  +  Se1[b,v,k,j,t]  ; 
                H2[b,v,k,j,t] =  (deltaT[k] * D2[b,v,k,j,t])  +  Se2[b,v,k,j,t]  ; 
                H34[b,v,k,j,t] = (deltaT[k] * D34[b,v,k,j,t]) +  Se34[b,v,k,j,t] ; 

            } else {

                H1[b,v,k,j,t] =  (deltaT[j] * D1[b,v,k,j,t])  +   Se1[b,v,k,j,t]  ; 
                H2[b,v,k,j,t] =  (deltaT[j] * D2[b,v,k,j,t])  +   Se2[b,v,k,j,t]  ; 
                H34[b,v,k,j,t] = (deltaT[j] * D34[b,v,k,j,t]) +   Se34[b,v,k,j,t] ; 
...
for(b in 1:B)
 for(v in 1:V)
  for(k in 1:K)
   ...
   Se[b,v,k,j,4]  = sum(Se1[b,v,k,j,37:T_De])  + sum(Se2[b,v,k,j,37:T_De])  ;
   
   Se[b,v,k,j,1]  += sum(Se34[b,v,k,j,1:13])  ;
   Se[b,v,k,j,2]  += sum(Se34[b,v,k,j,14:24]) ;
   Se[b,v,k,j,3]  += sum(Se34[b,v,k,j,25:36]) ;
   Se[b,v,k,j,4]  += sum(Se34[b,v,k,j,37:T_De])  ;
   }
  ...
  for(k in 1:K)
   for(j in 1:J_De) 
    for(d in 1:D_De){
          ...
          Se_BVKJD_De[b,v,k,j,d] = Se[b,v,k,j,d] * pop_HOSP_VJD_De[v,j,d]; // hosp across trial 
            }
...
for(b in 1:B)
 for(v in 1:V) 
  for(j in 1:J_De) 
   for(d in 1:D_De)
    Se_BVJD_De[b,v,j,d] = sum(Se_BVKJD_De[b,v, ,j,d]);

for(b in 1:B) 
 for(v in 1:V) 
  for(d in 1:D_De)
   Se_BVD[b,v,d] = sum(Se_BVJD_De[b,v, ,d]); 

for(b in 1:B) 
 for(v in 1:V)
 Se_BV[b,v] = sum(Se_BVD[b,v, ]);

for(b in 1:B) 
 for(d in 1:D_De)
 Se_BD[b,d] = sum(Se_BVD[b, ,d]);    

for(d in 1:D_De)
Se_SN_D[d] = Se_BD[1,d] ; // SN over time 

for(d in 1:D_De)
 Se_D[d] = sum(Se_BD[ ,d]); // total over time 
 
Se_total = sum(Se_D); // total 
...
for(b in 1:B)
 for(v in 1:V)
  ...
  pSe_BVJ[b,v,j]    = sum(Se_BVJD_De[b,v,j, ])  / Se_BV[2,2] ; // to get vac SP severe by age, its the age distribution within SP vaccinated I want
  pSe_BVKJ[b,v,k,j] = sum(Se_BVKJD_De[b,v,k,j, ])  / (Se_BV[1,1] + Se_BV[1,2] + Se_BV[2,1]) ;  //  not vac SP severe
  pSe_BVJD[b,v,j,d] = sum(Se_BVKJD_De[b,v, ,j,d])  / Se_BD[1,d] ; // SN severe only  
  }
...
for(j in 1:J_De)
 ...
 for(k in 1:K){
   ...
   for(k in 1:K){
    mSe_BVKJ_De[k] =    pSe_BVKJ[1,1,k,1] ; // SN C < 9
    mSe_BVKJ_De[k+4] =  pSe_BVKJ[1,1,k,2] ; // SN C > 9
    mSe_BVKJ_De[k+8] =  pSe_BVKJ[1,2,k,1] ; // SN V < 9
    mSe_BVKJ_De[k+12] = pSe_BVKJ[1,2,k,2] ; // SN V > 9
    mSe_BVKJ_De[k+16] = pSe_BVKJ[2,1,k,1] ; // SP C < 9
    mSe_BVKJ_De[k+20] = pSe_BVKJ[2,1,k,2] ; // SP C > 9
  } // No SP V 

  for(j in 1:J_De)
   for(d in 1:D_De){   
   mSe_SN_VJD_De[j,d]     =  pSe_BVJD[1,1,j,d]; // SN C 
   mSe_SN_VJD_De[j+2,d]   =  pSe_BVJD[1,2,j,d]; // SN V
    } // no SP 

  for(j in 1:J_De) 
  mSe_SPvJ_De[j] =  pSe_BVJ[2,2,j] ; // SP V  
  
 pC_De = Sy / pop_VCD_De; 
 pSe_De = Se_total / pop_HOSP_De; 
 
for(d in 1:D_De){
  pH_De[d] = Ho_D[d] / pop_HOSP_D_De[d]; 
  pSe_SN_D_De[d] = Se_SN_D[d] / sum(pop_HOSP_SN_VD_De[ ,d]) ; 
  }
...
// log likelihood 
 ...
 ll_De += multinomial_lpmf(VCD_VK_De  | mD_VK_De) ;
  ...
  ll_De += binomial_lpmf(SE_De | pop_HOSP_De, pSe_De); // total 
  for(d in 1:D_De) ll_De += binomial_lpmf(SE_SN_D_De[d] | sum(pop_HOSP_SN_VD_De[ , d]), pSe_SN_D_De[d]); // total SN over time
  for(d in 1:D_De) ll_De += multinomial_lpmf(SE_SN_VJD_De[,d] | mSe_SN_VJD_De[,d]) ; // SN only
  ll_De += multinomial_lpmf(SE_BVKJ_De   | mSe_BVKJ_De ) ; // No SP V 
  ll_De += multinomial_lpmf(SE_SPvJ_De   | mSe_SPvJ_De) ; // SP V by age 
...
array [B,V,K,J_De] real<lower = 0 > S_AR_BVKJ_De ; // no SP V 
array [V,J_De,D_De]   real<lower = 0 > S_AR_snVJD_De ; // SN only 
array [J_De]       real<lower = 0 > S_AR_spvJ_De ; // SP V only
...
vector[J_Q+ 2*D_Q+B*V*K*D_Q+2*B*V*J_Q*A+K*J_Q*4+B*V*K*4 + 2*D_De + 3 + 3 + 3 + 2*D_De + 6] log_lik; // plus 6 for Butantan-DV
...
// Dengvaxia severe 
for(d in 1:D_De) log_lik[d + 2*D_De + 3 + 3+ 319 + 6] = binomial_lpmf(SE_SN_D_De[d] | sum(pop_HOSP_SN_VD_De[ ,d]), pSe_SN_D_De[d]); 
log_lik[3*D_De + 3 + 3 + 1+ 319 + 6] = binomial_lpmf(SE_De | pop_HOSP_De, pSe_De); 
log_lik[3*D_De + 3 + 3 + 2+ 319 + 6] =  multinomial_lpmf(SE_BVKJ_De   | mSe_BVKJ_De ) ; 
for(d in 1:D_De) log_lik[d + 3*D_De + 3 + 3 + 2+ 319 + 6] = multinomial_lpmf(SE_SN_VJD_De[,d] | mSe_SN_VJD_De[,d]) ; 
log_lik[4*D_De + 3 + 3 + 3+ 319 + 6] = multinomial_lpmf(SE_SPvJ_De   | mSe_SPvJ_De) ; 
 ...
 for(v in 1:V)
  for(k in 1:K)
   for(j in 1:J_De){
    ...
    S_AR_BVKJ_De[b,v,k,j] = sum(Se_BVKJD_De[b,v,k,j, ]) / mean(pop_HOSP_BVJD_De[b,v,j, ]); 
   }
...
for(b in 1:B)
 for(v in 1:V)
  for(j in 1:J_De)
   for(d in 1:D_De)
     S_AR_snVJD_De[v,j,d] = Se_BVJD_De[1,v,j,d] / pop_HOSP_BVJD_De[1,v,j,d]; // SN only 

for(j in 1:J_De)
  S_AR_spvJ_De[j] = sum(Se_BVJD_De[2,2,j, ]) / mean(pop_HOSP_BVJD_De[2,2,j, ]); // SP V only, mean over time 
