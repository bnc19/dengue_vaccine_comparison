data{
   ...
   array[D] int<lower=0> SE_SN_D;  // total severe SN over time
   int<lower=0> SE;                // total severe (not published over time)
   ...
   array[(B*V*J*K)-8] int<lower=0> SE_BVKJ ;  // severe
   array[V*J,D]     int<lower=0> SE_SN_VJD;   // severe
   array[J]         int<lower=0> SE_SPvJ ;    // severe
...
for(v in 1:V)
 for(j in 1:J)
  for(d in 1:D){
    ...
    pop_HOSP_SN_VD[v,d] = sum(pop_HOSP_BVJD[1,v, ,d]) ; // SN by vaccine arm over time 
}
}
...
parameters{
  ...
  array [J] real<lower = 0, upper = 1> psi ;  // prob severe 
...
transformed parameters{
  ...
  array[B,V,K,J,D] real<lower = 0> Se_BVKJD;         // all cases 
  array[B,V,J,D]  real<lower = 0> Se_BVJD;
  ...
  vector [B*V*K*J-8]  mSe_BVKJ ;  
  matrix [V*J,D]   mSe_SN_VJD ;    
  vector [J]       mSe_SPvJ ;  
  ...
  array [B,V,K,J,T] real<lower = 0> Se1;
  array [B,V,K,J,T] real<lower = 0> Se2;
  array [B,V,K,J,T] real<lower = 0> Se34;
  ...
  array[D] real<lower=0, upper=1> pSe_SN_D; 
  real<lower=0, upper=1> pSe; 
  ...
  array [B,V,D]   real Se_BVD;
  array [B,V]     real Se_BV;
  array [B,D]     real Se_BD;
  array [D]       real Se_SN_D; // SN over time 
  array [D]       real Se_D;
  real Se_total; 
  ...
  array [B,V,J,D] real pSe_BVJD;
  array [B,V,K,J] real pSe_BVKJ;
  array [B,V,J]   real pSe_BVJ; 
...
if(psi_J ==1){
  for(j in 1:J) psiT[j] = psi[j];
} else {
    for(j in 1:J) psiT[j] = psi[1];
}
      ...
      for(t in 1:T){
         ...
         for(k in 1:K){
           ...
           D34[b,v,k,j,t] = (Inc3[b,v,k,j,t] + Inc4[b,v,k,j,t]) * gammaT[k] * phiT[k] * RR_symp[3,v,k,j,t];  
              ...
              Se1[b,v,k,j,t] = Inc1[b,v,k,j,t] * gammaT[k] * RR_hosp[1,v,k,j,t] * psiT[j] ; 
            if(include_eps == 0){
              Se2[b,v,k,j,t] = Inc2[b,v,k,j,t] * gammaT[k] * rhoT[k] * RR_hosp[2,v,k,j,t] * psiT[j]  ; 
            } else {
              Se2[b,v,k,j,t] = Inc2[b,v,k,j,t] * gammaT[k] * rhoT[k] * RR_hosp[2,v,k,j,t] * psiT[j]  * epsilon;  
            }
            Se34[b,v,k,j,t] = (Inc3[b,v,k,j,t] + Inc4[b,v,k,j,t])  * gammaT[k] * phiT[k] * RR_hosp[3,v,k,j,t] * psiT[j]  ; 
         }}}
         
for(b in 1:B)
  for(v in 1:V)
   for(k in 1:K)
    for(j in 1:J)  // hosp is severe cases plus prop non-severe (delta) hosp 
     for(t in 1:T){      
            if(delta_KJ != 2){
...
for(b in 1:B)
 for(v in 1:V)
  for(k in 1:K)
   ...
   Se[b,v,k,j,4]  += sum(Se34[b,v,k,j,37:T])  ;
   }
  }
  ...
  for(k in 1:K)
   for(j in 1:J) 
    for(d in 1:D){
          ...
          Se_BVKJD[b,v,k,j,d] = Se[b,v,k,j,d] * pop_HOSP_VJD[v,j,d]; // hosp across trial 
            }
...
for(b in 1:B)
 for(v in 1:V) 
  for(j in 1:J) 
   for(d in 1:D)
    Se_BVJD[b,v,j,d] = sum(Se_BVKJD[b,v, ,j,d]);
    

for(b in 1:B) 
 for(v in 1:V) 
  for(d in 1:D)
   Se_BVD[b,v,d] = sum(Se_BVJD[b,v, ,d]); 
...
for(b in 1:B)
 for(v in 1:V)
  ...
  pSe_BVJ[b,v,j]    = sum(Se_BVJD[b,v,j, ])     / Se_BV[2,2] ; // to get vac SP severe by age, its the age distribution within SP vaccinated I want 
  pSe_BVKJ[b,v,k,j] = sum(Se_BVKJD[b,v,k,j, ])  / (Se_BV[1,1] + Se_BV[1,2] + Se_BV[2,1]) ;  //  not vac SP severe
  pSe_BVJD[b,v,j,d] = sum(Se_BVKJD[b,v, ,j,d])  / Se_BD[1,d] ; // SN severe only  
  }
...
for(j in 1:J)
 ...
 for(k in 1:K){
   ...
   for(k in 1:K){
    mSe_BVKJ[k] =    pSe_BVKJ[1,1,k,1] ; // SN C < 9
    mSe_BVKJ[k+4] =  pSe_BVKJ[1,1,k,2] ; // SN C > 9
    mSe_BVKJ[k+8] =  pSe_BVKJ[1,2,k,1] ; // SN V < 9
    mSe_BVKJ[k+12] = pSe_BVKJ[1,2,k,2] ; // SN V > 9
    mSe_BVKJ[k+16] = pSe_BVKJ[2,1,k,1] ; // SP C < 9
    mSe_BVKJ[k+20] = pSe_BVKJ[2,1,k,2] ; // SP C > 9
  } // No SP V 
  
  
  for(j in 1:J)
   for(d in 1:D){   
   mSe_SN_VJD[j,d]     =  pSe_BVJD[1,1,j,d]; // SN C 
   mSe_SN_VJD[j+2,d]   =  pSe_BVJD[1,2,j,d]; // SN V
    } // no SP 
  
  
  for(j in 1:J) 
  mSe_SPvJ[j] =  pSe_BVJ[2,2,j] ; // SP V  
  

 pC = Sy / pop_VCD; 
 pSe = Se_total / pop_HOSP; 
 
for(d in 1:D){
  pH[d] = Ho_D[d] / pop_HOSP_D[d]; 
  pSe_SN_D[d] = Se_SN_D[d] / sum(pop_HOSP_SN_VD[ ,d]) ; 
}
...
// log likelihood 
 ...
 // Symp 
  ...
  // severe 
  ll += binomial_lpmf(SE | pop_HOSP, pSe); // total 
  for(d in 1:D) ll += binomial_lpmf(SE_SN_D[d] | sum(pop_HOSP_SN_VD[ , d]), pSe_SN_D[d]); // total SN over time
  
  for(d in 1:D) ll += multinomial_lpmf(SE_SN_VJD[,d] | mSe_SN_VJD[,d]) ; // SN only
  ll += multinomial_lpmf(SE_BVKJ   | mSe_BVKJ ) ; // No SP V 
  ll += multinomial_lpmf(SE_SPvJ   | mSe_SPvJ) ; // SP V by age 
...
generated quantities{
  ...
  array [B,V,K,J] real<lower = 0 > S_AR_BVKJ ; // no SP V 
  array [V,J,D]   real<lower = 0 > S_AR_snVJD ; // SN only 
  array [J]       real<lower = 0 > S_AR_spvJ ; // SP V only
...
// log lik
 ...
 for(d in 1:D) log_lik[d + 2*D + 3 + 3] = binomial_lpmf(SE_SN_D[d] | sum(pop_HOSP_SN_VD[ ,d]), pSe_SN_D[d]); 
  log_lik[3*D + 3 + 3 + 1] = binomial_lpmf(SE | pop_HOSP, pSe); 
 log_lik[3*D + 3 + 3 + 2] =  multinomial_lpmf(SE_BVKJ   | mSe_BVKJ ) ; 
 for(d in 1:D) log_lik[d + 3*D + 3 + 3 + 2] = multinomial_lpmf(SE_SN_VJD[,d] | mSe_SN_VJD[,d]) ; 
 log_lik[4*D + 3 + 3 + 3] = multinomial_lpmf(SE_SPvJ   | mSe_SPvJ) ; 
 ...
 for(v in 1:V)
  for(k in 1:K)
   for(j in 1:J){
    ...
    S_AR_BVKJ[b,v,k,j] = sum(Se_BVKJD[b,v,k,j, ]) / mean(pop_HOSP_BVJD[b,v,j, ]); 
}
...
for(b in 1:B)
 for(v in 1:V)
  for(j in 1:J)
   for(d in 1:D)
     S_AR_snVJD[v,j,d] = Se_BVJD[1,v,j,d] / pop_HOSP_BVJD[1,v,j,d]; // SN only 

  for(j in 1:J)
    S_AR_spvJ[j] = sum(Se_BVJD[2,2,j, ]) / mean(pop_HOSP_BVJD[2,2,j, ]); // SP V only, mean over time 
